...
\item if \(u\) is an ancient solution such that \(\|u\| \leq R_0\), and \(\partial_t u \not\equiv 0\), then \(u \in \mathcal{M}\).
\end{enumerate}

The graph \(\mathcal{M}\) is known as the \emph{unstable manifold}. It is invariant under the flow \eqref{eq:mcf} in the sense that if \(u_0 \in \mathcal{M}\) and \(u(t)\) is the solution of \eqref{eq:mcf} with \(u(0) = u_0\), then \(u(t) \in \mathcal{M}\). Thus \(\mathcal{M}\) contains all non-static, ancient solutions emanating from \(M_0\).

Since \(E_{\sigma^+}\) is non-empty, as noted in \Cref{thm:ancient_existence}, ancient solutions emanating from \(M_0\) certainly exist. The previous discussion shows that these ancient solutions are precisely the finite dimensional manifold \(\mathcal{M}\). Since the first eigenvalue is simple, we have the following uniqueness result.
