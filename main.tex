...
In general, all ancient solutions emanating from \(M_0\) are contained the in the center-unstable manifold,
\[
\{\Gamma (u) : L u \leq 0\}
\]
where \(\Gamma\) is a graph. In particular, null eigenfunctions (\(L u = 0\)) correspond to static solutions \(u \equiv 0\) is \(M_0\) itself), and a linear combinations \(a u + b v\) with \(L u = 0\), \(L v < 0\) corresponds to ancient solutions emanating from the static solution \(M_u = \Gamma(u)\). Thus in particular, the ancient solutions emanating from \(M_0\) (\(u \equiv 0\)) correspond to \(\{\Gamma(\sum a_i v_i) : Lv_i < 0\}\) and if the space \(\{v : L v < 0\}\) has dimension greater than one, there are many such solutions. Since the first eigenvalue is simple, we do however, have the following uniqueness result.

\begin{thm}
\label{thm:ancient_meanconvex_uniqueness}
There exists a unique, mean-convex, ancient solution emanating from \(M_0\).
\end{thm}
